...
Probublání prvku haldy nahoru je operace nad haldou, která dokáže v čase $\mathcal{O}(\log(n))$ opravit haldové uspořádání za předpokladu, že se snížila hodnota pouze jednoho vrcholu.

Algoritmus předpokládá, že haldové uspořádání je možné porušit pouze mezi jednou dvojicí vrcholů $u$ a $v$, kde $u$ je rodičovský vrchol a $v$ je jeho potomek. Tedy v celé haldě s výjimkou této dvojice musí platit haldové uspořádání. V této dvojici může platit, že syn~$v$ má menší hodnotu než jeho otec~$u$. Dále se předpokládá, že tento algoritmus bude proveden pouze v případě, že se hodnota některého vrcholu zmenšila, tudíž je potřeba tento vrchol probublat nahoru, kde se nacházejí vrcholy s menšími hodnotami. Pokud by se hodnota zvýšila, měl by být aplikován algoritmus probublání dolů, o kterém pojednává sekce \ref{subsec:bubbledown}.
...
Pro důkaz korektnosti tohoto algoritmu jsou použity \textit{řezy} haldou. Tyto řezy slouží pro rozdělení grafu haldy na dva podgrafy, ve kterých platí haldové uspořádání. Oba řezy dohromady tvoří původní graf haldy bez jedné problematické hrany $(u, v)$, ve které nemusí platit haldové uspořádání.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[level/.style={sibling distance=60mm/#1}]
		...
		child[thick,draw=blue] { node [circle,draw,fill=cyan!15] (1) {4}
			...
			child[draw=black] { node [circle,draw,very thick] (4) {5}
				child { node [circle,draw] (9) {7} }
	...
	\end{tikzpicture}
	\caption{Ukázka vrchního řezu haldy pomocí potomka}\label{img:heap-upper-child-cut}
\end{figure}
...
ACSL kód popisující vrchní řez haldy pomocí potomka nevytváří nový podgraf s~výše definovanými vlastnostmi, ale pouze ověřuje, že daná část (podgraf) haldy splňuje haldové uspořádání. Takto vytvořený predikát je následně použit v kontraktu funkce jako jedna ze vstupních podmínek algoritmu probublání nahoru a je také použit jako invariant cyklu. Predikát je tedy platný při zavolání funkce, po každém kroku cyklu a je také platný na konci vykonávání funkce. Tato induktivní vlastnost následné napomáhá při dokončení důkazu korektnosti algoritmu probublání nahoru.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[level/.style={sibling distance=60mm/#1}]
		...
		child[thick] { node [circle,draw] (1) {4}
			...
			child { node [circle,draw=blue,fill=cyan!15,very thick] (4) {5}
...
\begin{listing}[H]
	\caption{Kontrola tranzitivního haldového uspořádání mezi řezy haldou}
	\begin{minted}{c}
/*@
...
Indukcí lze dokázat, že při probublávání problematického vrcholu nahoru může vznikat  pouze jedna nová problematická dvojice a původní problematická dvojice má již haldové uspořádání opraveno. Původní problematická hrana se v dalším kroku cyklu vyskytuje v spodním řezu haldy pomocí potomka. Tedy pro ni platí haldové uspořádání.

Spojením horního a spodního řezu haldou a tranzitivního haldového uspořádání je možné formálně dokázat korektnosti algoritmu probublání prvku nahoru. Jelikož tyto vlastnosti platí na konci každého kroku cyklu, při dokončení cyklu se nacházíme v jednom z těchto stavů:
	...
	\begin{tikzpicture}[level/.style={sibling distance=60mm/#1}]
		...
		child[thick,draw=blue] { node [circle,draw,fill=cyan!15] (1) {4}
			...
			child { node [circle,draw,,fill=cyan!15] (4) {5}
				...
				child[draw=black] { node [circle,draw,very thick] (10) {1} }
			}
		}
		child[thick,draw=blue] { node [circle,draw,fill=cyan!15] (2) {5}

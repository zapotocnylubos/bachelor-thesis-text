...
Tato kapitola shrnuje základní pojmy teorie grafů, které zakládají  teoretické definice pro.
...
ACSL je akronym pro "ANSI/ISO C Specification Language". Algortmy v jazyce C lze anotovat pomocí jazyka ACSL a následně provádět statickou a formální verifikaci korektnosti daného kódu. Anotace je speciální typ komentáře v jazyce C, který začíná znakem '@'. Tedy '//@' pro jednořádkové definice a '/*@' pro víceřádkové definice. \cite{ACSLSpecification117}

Mezi časté anotace patří kontrakt funkce, který se nachází před deklarací funkce a popisuje chování funkce. Popisuje pravidla, které vstupní argumenty musejí splňovat, dále popisuje místa v paměti, které funkce může měnit a také popisuje vlastnosti proměnných, části paměti či návratové hodnoty, které platí po ukončení běhu funkce. Další důležitou anotací je invariant cyklu, který pomáhá deduktivní analýze dokončit induktivní důkaz platnosti nějaké vlastnosti, plynoucí z průběhu cyklu.

Popis vlastností lze seskupit pod predikát. Lze tedy založit nový predikát reprezentující nějakou vlastnost parametrů daného predikátu a následně ve zdrojovém kódu používat pouze tento predikát. \cite{ACSLSpecification117}


\subsection{WP}

\textit{Weakest precodition} plugin verifikačního prostředí Frama-C používá externí rozhodovací procedury pro dokázání ACSL anotací daných funkcí v programovacím jazyce C. \cite{FramaC24WPManual}

Hoareova trojice je způsob popisu algoritmů, který popisuje  výstupní vlastnosti proměnných, paměti a návratové hodnoty po provedení algoritmu, za předpokladu, že vstupní podmínky byly splněny. Obecně lze takovou trojici zapsat jako "{P} stmt {Q}". Kde \textit{P} jsou vstupní podmínky algoritmu, \textit{stmt} je obsah funkce implementující daný algoritmus a \textit{Q} jsou výstupní vlastnosti, které budou po ukončení volání funkce zajištěny. \cite{FramaC24WPManual}
...
RTE anotace nemusejí být součástí zdrojového kódu, ale mohou být vygenerovány až při spuštění grafického prostředí nebo pomocí příkazové řádky za pomocí přepínače "-rte".


\subsection{Smoke testy}

\textit{Smoke} testy jsou součástí speciální kontroly zdrojového kódu pomocí statické analýzy. Vstupní podmínky definované anotacemi ACSL mohou zapříčinit, že Frama-C a WP plugin dokáží dokončit formální důkaz některé vlastnosti algoritmu, které není možné dosáhnout, jelikož jsou vstupní podmínky nesplnitelné. Základním postupem pro kontrolu nesplnitelnosti vstupních podmínek je kontrola vstupních podmínek a invariantů cyklů za pomoci testu dokazatelnosti kontradikce. Pokud WP dokáže v některém bodě důkazu úspěšně dokázat splnitelnost false, testy selhávají s chybovou hláškou. \cite{FramaC24WPManual}

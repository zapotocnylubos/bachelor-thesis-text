...
Pro důkaz korektnosti tohoto algoritmu jsou použity \textit{řezy} haldou. Tyto \textit{řezy} slouží pro rozdělení grafu haldy na dva podgrafy, ve kterých platí haldové uspořádání. Oba řezy dohromady tvoří původní graf haldy bez jedné problematické hrany $(u, v)$, kde nemusí platit haldové uspořádání.
...
\begin{remark}
	V grafu vrchního řezu haldy pomocí potomka stále platí haldové uspořádání.
\end{remark}

ACSL kód popisující vrchní řez haldy pomocí potomka nemusí vytvářet nový podgraf s~výše definovanými vlastnostmi, ale stačí pouze ověřit, že daná část (podgraf) haldy splňuje haldové uspořádání. Takto vytvořený predikát je následně použit v kontraktu funkce jako jedna ze vstupních podmínek algoritmu probublání nahoru a je také použit jako invariant cyklu. Tento predikát je tedy platný při zavolání funkce, po každém provedení cyklu a je také platný na konci vykonávání funkce. Tato induktivní vlastnost následné napomáhá při dokončení důkazu korektnosti algoritmu probublání nahoru.
...
\begin{remark}
	V grafu spodního řezu haldy pomocí potomka stále platí haldové uspořádání.
\end{remark}
...
Indukcí lze následně dokázat, že při probublávání problematického vrcholu nahoru může vznikat  pouze jedna nová problematická dvojice a původní problematická dvojice má již haldové uspořádání opraveno a dané hrana se přidá v dalším kroku algoritmu do spodního řezu haldy pomocí potomka.
...
\begin{remark}
	V grafu vrchního řezu haldy pomocí rodiče stále platí haldové uspořádání.
\end{remark}
...
\begin{remark}
	V grafu spodního řezu haldy pomocí rodiče stále platí haldové uspořádání.
\end{remark}

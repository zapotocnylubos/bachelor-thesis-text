...
Vrchním řezem haldy pomocí potomka nazveme graf $G' = (V', E')$, kde $V' = \{ v: v \in V \land k(v) < i \}$ a~$E' = \{ (u, v): (u, v) \in E \land u \in V' \land v \in V' \}$.
\end{definition}

\begin{definition}[Spodní řez haldy pomocí potomka]
Mějme graf $G = (V, E)$ reprezentující binární haldu a~$i \in \mathbb{N}$.
Spodním řezem haldy pomocí potomka nazveme graf $G' = (V', E')$, kde $V' = \{ v: v \in V \land k(v) > i \} \cup \{ Parent(v): v \in V \land k(v) > i \}$ a~$E' = \{ (u, v): (u, v) \in E \land u \in V' \land v \in V' \}$.
\end{definition}
